...
Possible values of `type`
 ...
 - `gtCityWalks`  An extension of the pixel-wise ground truths, by re-annotating the class `person` in all training and validation images with additional labels corresponding to four pedestrian’s body orientations (`back`, `right`, `front`, and `left`). It is constructed to challenge the `Attribute-aware Semantic Segmentation task` aimed at understanding the pedestrians in a traffic scene. Please, kindly refer to [CityWalks and Attribute-aware Semantic Segmentation (Sulistiyo, et al., IEICE Transactions 2020)](https://www.jstage.jst.go.jp/article/transfun/E103.A/1/E103.A_2019TSP0001/_article) for more details.
